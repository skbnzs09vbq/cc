...
import { LOCAL_DIR, PROJECT_TS_PATH, VERIFY_STATE_PATH } from '../../shared/paths.js'
import { dedent } from '../../shared/utils.js'

const TARGET_PATHS = ['project.example.ts', 'CLAUDE.md', 'agents', 'skills', 'src']
...
const SELF_PATH = 'src/verify/no-project-leak/skill.ts'
...
export function verifyNoProjectLeak(): string {
  ...
  const findings = candidates
    ? complete(
        dedent`
          以下は project.example.ts・CLAUDE.md・agents/・skills/・src/ 配下の走査で引っかかった箇所です
