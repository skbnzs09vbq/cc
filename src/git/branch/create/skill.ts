...
import { type Schema, askUser, exit, respond, runCommand } from '../../../shared/complete.js'
...
export const ARGS_SCHEMA = {
  ...
  properties: {
    ...
    branchName: {
      type: ['string', 'null'],
      description: 'string: 作成するブランチ名, null: 未定な場合',
    },
    baseBranch: {
      type: ['string', 'null'],
      description:
        'string: 分岐元ブランチ名（workDescription 中に明記があればそれ）, null: 明記が無ければ',
    },
    workDescription: {
      type: ['string', 'null'],
      description:
        'string: branchName が null の場合に、ブランチ名を決めるための作業内容の説明, null: branchName が指定済みの場合',
    },
  },
