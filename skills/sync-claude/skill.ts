...
export function syncClaude(args: Infer<typeof ARGS_SCHEMA>): string {
  const { path } = args
  let { confirmed } = args
  ...
  if (remoteExists !== 'yes') exit(`${remoteClaudePath} が見つかりません`)

  const remoteIsGit =
    runCommand([`test -d "${remoteClaudePath}/.git" && echo yes || echo no`])?.trim() === 'yes'
  ...
  const modified = common.filter(
    (f) =>
      runCommand([
        `diff -q "${remoteClaudePath}/${f}" ".claude/${f}" >/dev/null 2>&1 || echo diff`,
      ])?.trim() === 'diff',
  )
...
respond(syncClaude(getArgs(ARGS_SCHEMA)))
